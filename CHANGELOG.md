...
## [v2.0.0] - 2025-01-26 🚀 算法重大改进版本

### 🧠 算法核心修复与优化

#### ✅ SM2算法修复 (Critical Fixes)
- **修复间隔计算公式错误**: 移除错误的 `* (grade / 5)` 因子，严格按照Piotr Woźniak官方SM2规范
- **修复EF更新时机**: 失败复习时保持EF不变，只在成功时更新EF值
- **修复Grade=0参数验证**: 解决未定义EF导致的潜在错误
- **算法准确性提升**: 现在完全符合SuperMemo官方算法标准

#### 🚀 FSRS算法默认启用
- **默认算法切换**: 新用户默认使用FSRS算法而非SM2
- **科学依据**: FSRS基于500万+真实复习记录，预测准确性比SM2高15-20%
- **向后兼容**: 现有用户设置保持不变，可自由切换

### 🎯 优先级系统增强

#### ✅ 错误处理改进
- **用户反馈机制**: 保存失败时提供明确的用户提示
- **数据保护**: 保存失败时保留用户修改，支持重试
- **组件生命周期**: 防止组件卸载后的异步操作错误

#### ✅ 依赖管理优化
- **React依赖数组**: 修复所有useEffect的依赖数组，符合React最佳实践
- **参数验证**: 增强函数参数验证，提高系统稳定性

### 📊 代码质量提升
- **算法实现**: FSRS使用官方ts-fsrs库，实现完美规范
- **类型安全**: 改进TypeScript类型定义和边界检查
- **测试覆盖**: 算法修复通过多种边界情况测试

### 🔄 版本管理
- **回滚安全**: 所有更改都可以安全回滚到之前版本
- **分支管理**: 提供algorithm-improvements分支作为备份
- **提交记录**: 详细的提交历史便于问题追踪

### ⚠️ 重要提醒
- 算法切换不会影响现有学习数据
- 建议新用户使用默认的FSRS算法获得更好体验
- 如有问题可随时回滚到稳定版本

### 🙏 致谢
基于 digitalmaster/roam-memo 项目，感谢原作者的杰出工作！
